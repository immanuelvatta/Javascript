...
function insertionSort(nums =[]) {
    //start at index 1, we're looping through 1 to the end
    for(let i = 1; i < nums.length; i++){
        //we check it against the previous index
        for(let j = i; j > 0; j--){
            // if it is less then we swap
            if(nums[j] < nums[j-1]){
                //restructuring so we can swap j with j-1
